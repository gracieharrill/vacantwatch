...
function createPopupHtml(property: Property): string {
  ...
  const badges = propertySignals
    .map((signal) => {
      const color = statusColors[signal];
      const label = statusLabels[signal];
  ...
  const outstanding =
    property.outstandingAmount !== undefined
      ? `
  ...
  const assessedValue =
    property.totalAssessedValue !== undefined
      ? `
  ...
  const presentUse = property.presentUse
    ? `
      <div style="margin-top: 6px; color: #475569;">
        ${escapeHtml(property.presentUse)}
      </div>
    `
    : "";
  ...
  return `
    ...
    >
      ...
      <div
        ...
        "
      >
        Parcel ID: ${escapeHtml(property.id)}
      </div>
    </div>
  `;
}
...
export default function Map({
  ...
  const onSelectPropertyRef =
    useRef(onSelectProperty);

  useEffect(() => {
    onSelectPropertyRef.current =
      onSelectProperty;
  }, [onSelectProperty]);
  ...
  useEffect(() => {
    const container =
      containerRef.current as LeafletContainer | null;
